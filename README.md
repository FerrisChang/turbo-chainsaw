...
.company-submission-frame>.input, .alt-id-submission-frame>.input {
  display: flex; 
  flex-direction: row;
}
.alt-id-submission-frame>.input {
  flex: 2; 
}
.clear-button-frame {
  flex: 0; 
...
.input-content.ng-touched.ng-invalid {
  border-color: red; 
  border-width: 2px;
}
.role-frame {
...
<div class="validation-container very-front" *ngIf="input.length > 0">
